...
{
	//try checks if the record is invalid
	//this try catch isn't really needed, but it catches values that may slip through
	try {
		if (index <= records.size() - 1) {
			return records[index];
		}
		else { throw (index); } //if invalid, throw
	}
	catch (int num){ //catches bad indexes and returns the previous index
...
void DataFile::Save(string filename)
{
	...
	int recordCount = records.size(); //sets the record count to the amount of records
	outfile.write((char*)&recordCount, sizeof(int)); //writes the record count out 

	for (int i = 0; i < recordCount; i++)
	{		
		Color* imgdata = GetImageData(records[i]->image); //gets the image data
...
void DataFile::Load(string filename)
{
	...
	for (int i = 0; i < recordCount; i++)
	{
		...
		r->image = img;
		r->name = string(name);
		//brute force solution -- deletes the extra characters until the size is what was defined, works while I try find the intended solution
