...
void Initialise() {
	...
	for (int i = 0; i < asteroidMax; i++) {
		...
		asteroid[i].position = Vector2{ (float)randX, (float)randY };
		...
		asteroid[i].direction = Vector2{ (float)dirX, (float)dirY };
	...
	for (int i = 0; i < chonkAsteroidMax; i++) {
		...
		chonkAsteroid[i].color = EVENLIGHTERGRAY;
	}
...
void Update() {
	if (!victory) {
		if (!gameOver) {
			...
			for (int i = 0; i < smolAsteroidMax; i++) {
				if (CheckCollisionCircles(Vector2{ player.position.x + sinf(player.rotation * DEG2RAD) * (player.shipSize * 0.5f), player.position.y - cosf(player.rotation * DEG2RAD) * (player.shipSize * 0.5f) },
...
void Draw() {
	...
	if (!gameOver) {
		...
		for (int i = 0; i < asteroidMax; i++) {
			if (asteroid[i].active) DrawCircleV(asteroid[i].position, asteroid[i].radius, asteroid[i].color);
		}
		for (int i = 0; i < chonkAsteroidMax; i++) {
			if (chonkAsteroid[i].active) DrawCircleV(chonkAsteroid[i].position, chonkAsteroid[i].radius, chonkAsteroid[i].color);
		}
		...
		if (victory) DrawText("VICTORY", 700, 450, 80, WHITE);
		...
		//draws the hitbox of the player
		//DrawCircleV(Vector2{ player.position.x + sinf(player.rotation * DEG2RAD) * (player.shipSize * 0.5f), player.position.y - cosf(player.rotation * DEG2RAD) * (player.shipSize * 0.5f) }, player.collisionRad, RED);
	}
	else DrawText("GAME OVER", 600, 450, 60, WHITE); //gameover text
